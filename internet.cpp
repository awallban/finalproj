...
public:
		...
		bool checkWin(Player player){
			for(unsigned int i = 0; i <3; i++){
				//check the horizontal scenario
				if(board[i][0]==player && board[i][1]==player && board[i][2]==player){
					return true;
				}
				//check the vertical scenario
			...
			void play(){
				...
				printBoard();
				std::cout<<"Enter your move choice"<<endl; 
				...
				do{
					...
					if(turn==0){
						getHumanMove();
						if(checkWin(Player::human)){
							std::cout<<"Human Wins!"<<endl;
							exit=true;
						}
					}
					else{
						std::cout<<endl<<"Computer move!"<<endl;
						Move aimove=minimax();
						std::cout<<aimove.x<<aimove.y<<endl;
